...
const CreatePin = ({ user }) => {
  ...
  const uploadImage = (e) => {
    ...
    if(type === 'image/png' || type === 'image/svg' || type === 'image/jpeg' || type === 'image/gif' || type === 'image/tiff'){
      ...
      client.assets
        .upload('image', e.target.files[0], { contentType: type, fileName: type.name })
        .then((document) => {
      ...
      <div className="flex lg:flex-row flex-col justify-center items-center bg-white lg:p-5 p-3 lg:w-4/5 w-full">
        ...
        <div className="flex flex-1 flex-col gap-6 lg:pl-5 mt-5 w-full">
          ...
          <input 
            ...
            className="outline-none text-base sm:text-lg border-b-2 border-gray-200 padding-2"
          />
          ...
          <div className="flex flex-col">
            <div>
              ...
              <select onChange={(e) => setCategory(e.target.value)} className="outline-none w-4/5 text-base border-b-2 border-gray-200 p-2 rounded-md cursor-pointer">
                <option value="others" className="sm:text-bg bg-white rounded-md">Select Category</option>
                {categories.map((category) => (
                  <option className="text-base border-0 outline-none capitalize bg-white text-black rounded-md" value={category.name}>{category.name}</option>
                ))}
              </select>
            </div>
